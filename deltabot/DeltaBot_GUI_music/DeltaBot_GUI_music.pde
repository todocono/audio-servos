...
void setup() {
  ...
  //music switch
  controlP5.addToggle("music", false, 20, 100, 50, 20).setMode(ControlP5.SWITCH);
...
void draw() {
  ...
  if (musicEnabled){
     ...
     xp = Float.valueOf(mouseX);
   yp = Float.valueOf(mouseY);
  // zp = Float.valueOf(;
...
void setThetasfromXYZ() {
  if ( debug ) { 
    println("Entering: setThetasfromXYZ()");
  }
  ...
  updateGuiElements();
  if ( debug ) { 
    println("Exiting: setThetasfromXYZ()");
  }
}
...
void setXYZfromThetas() {
  if ( debug ) { 
    println("Entering: setXYZfromThetas()");
  }  
  validPosition = true;
  ...
  updateGuiElements();
  if ( debug ) { 
    println("Exiting: setXYZfromThetas()");
  }
}
...
void updateGuiElements() { 
  if ( debug ) { 
    println("Entering: updateGuiElements()");
  } 
  ...
  if ( debug ) { 
    println("Exiting: updateGuiElements()");
  }
}
...
void SetServos() {
  if ( debug ) { 
    println("Entering: SetServos()");
  }
  if (servosEnabled) {
    if (musicEnabled) {
        println("updating according to music");
...
void enable(boolean btnEnabled) {
  if ( debug ) { 
    println("Entering: enable() (servos)");
  }
  if ( btnEnabled==true ) {
  ...
  if ( debug ) { 
    println("Exiting: enable() (servos)");
  }
}

//enable or disable music detection
void music(boolean btnMusic) {
  if ( debug ) { 
    println("Entering: music() (servos)");
  }
  if ( btnMusic==true ) {
    println("music detection");
    musicEnabled = true;
  } else {
    sendMove(99, 0); //turn servos off
    musicEnabled = false;
